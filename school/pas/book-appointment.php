...
{
	$get_appointments = "select * from student_appointment";
	
	$run_appointments = mysqli_query($con,$get_appointments);
	
	$count = mysqli_num_rows($run_appointments);
	if($count >= 10){
		echo "<script>alert('Sorry the maximum appointment that can be booked has been reached :(..Please try again tomorrow');</script>";
	} else{
		$course = $_POST['course'];
		...
		$userstatus =  1;
		$supervisorstatus = 1;
		$query = "INSERT into student_appointment(course,supervisorName,email,matric,appdate,apptime,userStatus,supervisorStatus) 
		values ('$course','$supervisorName','$userEmail', '$matric_no','$appdate','$time','$userstatus','$supervisorstatus')";

		$result = mysqli_query($con,$query) or die("Error".mysqli_error($con));
			if($result){
				echo "<script>alert('Your appointment has successfully been booked');</script>";
			}
	}
...
<html lang="en">
	...
	<div id="app">		
		...
		<div class="form-group">
			...
			while($row = mysqli_fetch_array($sql)){
			?>
				<option value="<?php echo htmlentities($row['supervisorName']);?>">
					<?php echo htmlentities($row['supervisorName']);?>
				</option>
			<?php } ?>
			</select>
		</div>	
		...
		<div class="form-group">
			...
			while($row = mysqli_fetch_array($sql)){
			?>
			<input type="text" name="matric" class="form-control" value="<?php echo htmlentities($row['matric_no']);?>" readonly>
			<?php } ?>
		</div>	
		...
		<div class="form-group">
			...
			while($row = mysqli_fetch_array($sql)){
			?>
				<option value="<?php echo htmlentities($row['appointmentDate']);?>">
		...
		<div class="form-group">
			...
			while($row = mysqli_fetch_array($sql)){
			?>
				<option value="<?php echo htmlentities($row['appointmentTime']);?>">
